...
def download_report(reportDate, reportName, reportMimeType):
    # organizationId = "gphktoken"

    try:
        config_obj = configuration.Configuration()
        client_config = config_obj.get_configuration()
        api_instance = ReportDownloadsApi(client_config)
        if reportMimeType == 'text/csv':
            extension = '.csv'
        else:
            extension = '.xml'
        # api_instance.api_client.download_file_path = os.path.join(os.getcwd(), "resources", "download_report.csv")
        api_instance.api_client.download_file_path = "/Users/ken/Downloads/"+reportName+"_"+reportDate+extension
        # status, headers = api_instance.download_report(reportDate, reportName, organization_id=organizationId)
        status, headers = api_instance.download_report(reportDate, reportName)
...
if __name__ == "__main__":
    download_report("2023-08-20", "Daily_HK_report", "text/csv")
